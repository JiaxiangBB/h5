function fullCardTraDetailsCtrl($scope,$state,$stateParams,$ionicModal){
	...
	//console.log($scope.fullCardNo);

	//调用查询交易明细
	var _argumentsUrl = getRootPath()+"/AccountAction.accPreTradeQuery.do?FAPView=JSON";
	$scope.lists = [];
	$scope.day=[];//星期数组
	var page_no = 1;
	var page_size = 10;
	...
	$scope.$on('$ionicView.loaded', function(event, data) {
		...
		$(".list").html("");
		//调用AJAX封装函数
		get_goods_list(_argumentsUrl, {'customerNo':$scope.customerNo,'sessionId':$scope.sessionId,'beginDate':$scope.beginDate,'endDate':$scope.endDate,'beginPos':page_no,pageSize:page_size,accType:'03',orderBy:'3',trsType:"",cardNo:$scope.fullCardNo},function(data){
...
angular.module("myapp")
		.controller("fullCardTraDetailsCtrl",['$scope','$state','$stateParams','$ionicModal',fullCardTraDetailsCtrl]);