function paymentRecordsCtrl($scope,$stateParams,$http,$state,$ionicHistory,$ionicModal,$ionicLoading){
	//公共参数
	$scope.customerNo=getURLParameter("customerNo");
	$scope.sessionId=localStorage.getItem("data");

	//缴费记录图标
	$(".paymentRecords .water img").attr("src",imgRrc+"h5images/icon_life_water24"+variable+".png");
	$(".paymentRecords .electric img").attr("src",imgRrc+"h5images/icon_life_electric24"+variable+".png");
	$(".paymentRecords .gas img").attr("src",imgRrc+"h5images/icon_life_fire24"+variable+".png");
	//没有记录时的背景图
	...
	$scope.$on('$ionicView.loaded', function(event, data) {
		    ...
		    $scope.noMorePage=false;
			$(".list").html("");
	    	get_goods_list(_arguments, {'customerNo':$scope.customerNo,'sessionId':$scope.sessionId,'beginDate':$scope.beginDate,'endDate':$scope.endDate,'limit':page_size,orderBy:4,page:page_no,selectMark:"JF"},function(data){
...
angular.module("myapp")
		.controller("paymentRecordsCtrl",['$scope','$stateParams','$http','$state','$ionicHistory','$ionicModal','$ionicLoading',paymentRecordsCtrl]);
